...
│   └── requirements_bot.txt        Dependencies for the scheduler bot
├── src/
│   └── sim24_bot/
│       ├── booking.py
│       ├── captcha_handler.py
...
│       └── telegram_notify.py
├── dev_tools/
│   └── local_test_script.py        Manual runner implementation (not collected by pytest)
├── main.py                         Entry point (imports from `src/sim24_bot`)
├── local_test.py                   Manual test runner (CLI wrapper for local integration)
├── requirements.txt
├── pyproject.toml
└── .env.example
```
...
| `GITHUB_GIST_TOKEN`  | GitHub Personal Access Token used for the Gist |
| `GITHUB_GIST_ID`     | Gist ID created in Step 2                      |
...
```Shell
# Unit tests (run the pytest suite)
python -m pytest

# Manual/integration tests (local runner)
...
During history-rewrite or debugging runs some temporary artifacts may exist locally (for example `repo-mirror.git`, `.pytest_cache`, or `__pycache__`). They are safe to remove from your machine; using PowerShell:

```PowerShell
# Windows PowerShell
Remove-Item -LiteralPath 'repo-mirror.git' -Recurse -Force -ErrorAction SilentlyContinue
...
```Shell
rm -rf repo-mirror.git .pytest_cache __pycache__
```

The manual local runner implementation lives at `dev_tools/local_test_script.py`; use `python local_test.py` to invoke the CLI wrapper. For details about the history-rewrite procedure see [HISTORY\_REWRITE.md](HISTORY_REWRITE.md). If secrets were ever committed, rotate them immediately (see the runbook for guidance).
